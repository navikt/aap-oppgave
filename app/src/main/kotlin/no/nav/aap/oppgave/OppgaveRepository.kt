...
class OppgaveRepository(private val connection: DBConnection) {
    ...
    private fun oppgaveMapper(
        ...
        //  - Kafka-topic behandling_endres fra tilbake-løsning kan komme inn med ny eller oppdatert oppgave som skal vises med status påvent hvis grunn/gjnopptas-dato ikke er null
        //  - Utløpt påvent frist blir ikke varslet fra tilbake-løsninge, vi må selv sjekke ved henting av oppgaveliste om dagens dato har passert frist dato
        ...
        return when (behandlingstype) {
            // tilpass påvent-data til tilbakekreving oppgaver
            Behandlingstype.TILBAKEKREVING -> standardOppgaveDto.copy(
                ...
                forrigeVenteBegrunnelse = if (erFristUtløpt) row.getStringOrNull("VENTE_BEGRUNNELSE") else row.getStringOrNull("SISTE_VENTE_BEGRUNNELSE"),
                utløptVentefrist = utløptVentefrist ?: if (erFristUtløpt) påVentTil else null
            )
            // ellers bruk påvent-data som før
